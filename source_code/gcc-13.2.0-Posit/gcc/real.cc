...
//posit64 declaration&defined
static void encode_posit64(const struct real_format * , long* , const REAL_VALUE_TYPE *r);
static void decode_posit64(const struct real_format * ,REAL_VALUE_TYPE * , const long *);

inline void posit64Reverse(unsigned long &image_hi,unsigned long &image_lo){
  // 做 bit補數+1 運算

  image_hi = (~image_hi)&0xffffffff;
  image_lo = (~image_lo)&0xffffffff;
  //產生carry -> hi +1
...
static void encode_posit64_2(const struct real_format *fmt , long *buf , const REAL_VALUE_TYPE *r){
  ...
  switch(r->cl){
    ...
    case rvc_normal:
      ...
      else if (exp >= 248 && sign == 1)
      {
        image_hi = 0x80000000;
        image_lo = 0x00000001; //-maxpos
      }
      else if (exp <= -248 && sign == 0)
      {
        image_hi = 0x00000000;
        image_lo = 0x00000001; // minpos
      }
      else if (exp <= -248 && sign == 1)
      {
        image_hi = 0xFFFFFFFF;
        image_lo = 0xFFFFFFFF; //-minpos
      }
      ...
      else if (sign == 1 && exp == 0 && sig_lo == 0 && sig_hi == 0)
      {
        image_hi = 0xC0000000;
        image_lo = 0x00000000; //-1
      }
        ...
        if(fracLength < 0){ //no frac bits
          ...
          if(HOST_BITS_PER_LONG == 64){
            ...
            if ((r->sig[SIGSZ - 1] & (((unsigned long)1 << (64-2 - fracLength)) - 1)) > 0){
              bitsMore = 1;
            }
            else{
              bitsMore = 0;
            ...
            if(fracLength>=31){ //都看lowBits
            ...
            else if(fracLength == 30){ //看highBits + lowBits
              bitNPlusOne = (r->sig[SIGSZ-1] &1);
              if(r->sig[SIGSZ-2] > 0){
                bitsMore = 1;
              }
              else{
              ...
              bitNPlusOne = (r->sig[SIGSZ - 1] >> (32-2 - fracLength)) & 1;
              if( r->sig[SIGSZ-2]>0 ||  (( r->sig[SIGSZ - 1] & ((((unsigned long)1 << (32-2 - fracLength))-1))) >0) ){
                bitsMore = 1;
              }
              else{
                bitsMore = 0;
          ...
          //set frac bits max:59 bits = 27+32
          if(fracLength == 59){
            frac_hi = sig_hi;
            frac_lo = sig_lo;
          }
          else if(fracLength >32){
            int sigHi_at_fracHi = (fracLength -32);
            int sigHi_at_fracLo = 27 - sigHi_at_fracHi;

            frac_hi = sig_hi >> sigHi_at_fracLo;
            frac_lo = ( (sig_hi << (32- sigHi_at_fracLo)) | (sig_lo >> sigHi_at_fracLo) ) & 0xffffffff;
          }
          else if(fracLength >27){
            int sigLoBits = fracLength - 27; //use sig_lo "sigLoBits" at frac_lo; //value:1~6
            frac_hi = 0x0;
            frac_lo =( (sig_hi << sigLoBits) | (sig_lo >> (32-sigLoBits)) )  & 0xffffffff; // &一下，安全
          }
          else{
            frac_hi = 0x0;
            frac_lo = sig_hi >> (27 -fracLength) ;
          }
        }
        ...
        if(regLength <=60){
          ...
          if(regLength<=28){
            exp_hi = exp << (28 - regLength);
            exp_lo = 0x0;
          }
          else if(regLength >=30){
            exp_hi = 0x0;
            exp_lo = exp << (30 - (regLength-30));
          }
          else if(regLength == 29){
            exp_hi = exp >> 1;
        ...
        image_lo += ( (bitNPlusOne & (image_lo & 1) ) | (bitNPlusOne & bitsMore) );
        //bitNPlusOne & bitsMore判定End
...
static void encode_posit64(const struct real_format *fmt , long *buf , const REAL_VALUE_TYPE *r){
if(!fposit){
  ...
  switch(r->cl){
    ...
    case rvc_normal:
      ...
      else if (exp >= 496 && sign == 1)
      {
        image_hi = 0x80000000;
        image_lo = 0x00000001; //-maxpos
      }
      else if (exp <= -496 && sign == 0)
      {
        image_hi = 0x00000000;
        image_lo = 0x00000001; // minpos
      }
      ...
      else if (sign == 1 && exp == 0 && sig_lo == 0 && sig_hi == 0)
      {
        ...
        if(fracLength < 0){ //no frac bits
          ...
          if(HOST_BITS_PER_LONG == 64){
            ...
            if(fracLength>=31){ // (32-1[target bit] - (fracLength-31 [sig的1bit是hiddenBit,不列入計算])) 
              ...
              // 32-2 - fracLength [1 for target, 1 for hidden bit]
              bitNPlusOne = (r->sig[SIGSZ - 1] >> (32-2 - fracLength)) & 1;
              if( r->sig[SIGSZ-2]>0 ||  (( r->sig[SIGSZ - 1] & (((unsigned long)1 << (32-2 - fracLength))-1)) >0) ){
                bitsMore = 1;
              }
          ...
          if(fracLength == 58){
            ...
            //當58>fracLength>32: 必定使用滿frac_lo,frac_hi部分填滿,由於fracLength不一定使用全部sig,因此需再次為移到正確位置,並將未使用部分捨棄
            int sigHi_at_fracHi = (fracLength -32);
            int sigHi_at_fracLo = 26 - sigHi_at_fracHi;
        ...
        //if image_lo all bits is 1 will create carry bit,so need to extra add 1 at image_hi
        if(image_lo == 0xffffffff){
          image_hi += ( (bitNPlusOne & (image_lo & 1) ) | (bitNPlusOne & bitsMore) );
        }
        image_lo += ( (bitNPlusOne & (image_lo & 1) ) | (bitNPlusOne & bitsMore) );
...
static void decode_posit64_2(const struct real_format *fmt ,REAL_VALUE_TYPE *r , const long *buf){
  ...
  if(offset == -1){
    offset = 31 ;
    while(offset >=0){
      if(regimeFirstBit ^ ((image_lo>>offset) & 0x1) ){
        break;
      }
      else{
        ++regLength;
  ...
  if(regLength <= 60){        
    if(regLength<=28){ // 1 for regEndBit ,1 for signBit
      ...
      exp = (image_lo >> fracLength) & 0x3;
    }
    else if(regLength ==29){
      exp = (((image_hi & 0x1) << 1) | ((image_lo >> 31) & 0x1));
    }
  ...
  if(regLength == 63 && regimeFirstBit == 0 && sign == 1 && fmt->has_inf){
    r->cl = rvc_nan;
    r->sign = sign;
    r->signalling = 0; //turn to quiet NaN
...
if(!fposit){
  ...
  unsigned long image_hi;
  unsigned long image_lo;
  unsigned long regLength,es=3;

  long fracLength =0;
  //依照是Big or Little edian 決定誰是hi 誰是lo
  if (FLOAT_WORDS_BIG_ENDIAN){
    image_hi = buf[0], image_lo = buf[1];
  }
  else{
  ...
  decode_posit64_2(fmt,r,buf);
}
}

const struct real_format posit64_format ={
  //copy from posit32 for test
  encode_posit64,
  decode_posit64,
...
static void encode_ieee_single (const struct real_format *fmt, long *buf, const REAL_VALUE_TYPE *r)
{
...
static void decode_ieee_single (const struct real_format *fmt, REAL_VALUE_TYPE *r, const long *buf){
  unsigned long image = buf[0] & 0xffffffff;
  bool sign = (image >> 31) & 1;
