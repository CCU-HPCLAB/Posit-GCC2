...
riscv_option_override (void)
{
  ...
  /* Control whether use posit number to replace
     IEEE-754 floating-point format.
     Added by pschen. */
  if (posit_num_switch_flag) {
      //cpp_define ("__POSIT_NUM_SWITCH_FLAG__"); 
      flag = 0;
      REAL_MODE_FORMAT (SFmode) = &posit32_format;
      REAL_MODE_FORMAT (DFmode) = &posit64_format;
  }
}
